...
- [x] 🟢 Concluído — IA, segunda leva: **logs de `/logs`** lidos de forma inteligente (`get_logs_overview` agrupa as mensagens por padrão — números/IPs viram `#` — com contagem, severidade e padrões de erro em destaque; `search_logs` colapsa repetições); **análises** que já existiam para as telas (causa raiz pela topologia, comparação com o baseline, padrão por hora do dia com gráfico, linha do tempo do incidente juntando alertas, mudanças de status e logs); **ações com confirmação** (reconhecer/silenciar alerta, janela de manutenção, criar monitor — a IA propõe, o usuário confirma no chat, `POST /api/ai/tools/execute` executa pelos mesmos serviços da tela e audita em nome dele; `requireToolConfirmation` passou a valer para os testes ativos); **usabilidade** (conversas salvas no navegador, "Diagnosticar com IA" em alerta, dispositivo e monitor com o contexto da tela, consumo de tokens por resposta, histórico enviado limitado às últimas trocas); **IA proativa configurável e desligada por padrão** (resumo automático de incidente gravado no alerta e entregue por SSE, com severidade mínima e teto por hora; resumo diário/semanal da rede pelos canais de notificação, com geração sob demanda). Criação de monitor e ações de alerta saíram dos controllers para `services/monitoring/creation.rs` e `services/alerts/actions.rs`.
- [x] 🟢 Concluído — Histórico do Assistente IA na conta do usuário, acessível de qualquer computador: tabela `ai_conversations` (migration `m20260922_000001`, FK para `users` com `CASCADE`), CRUD em `/api/ai/conversations` sempre restrito ao dono (conversa alheia responde 404), teto de 100 conversas por usuário (as mais antigas saem) e de 2 MB por conversa. A tela migra sozinha as conversas que estavam no `localStorage` e depois as apaga do navegador; a lista é relida do servidor sempre que o histórico é aberto (sem polling).
- [x] 🟢 Concluído — `grep` da IA (substitui `search_logs`): procura em **logs**, **mensagens de alertas** e **falhas de checagem** sem trazer os dados — padrão literal ou regex (motor linear, com teto de tamanho), exclusão (`-v`), filtros de dispositivo/severidade/janela e cinco saídas, da mais barata à mais cara: `count` (`-c`, com distribuição por hora), `sources` (`-l`), `patterns` (mensagens distintas), `lines` (repetições colapsadas, trecho em volta do casamento em vez da mensagem inteira, `-C` de até 3 linhas da mesma origem) e `auto` (linhas se couberem, senão resumo com orientação para refinar). O banco pré-filtra por substring sem caixa (`LOWER(message) LIKE`, igual em SQLite e PostgreSQL) só quando o termo é ASCII e literal, para nunca perder uma ocorrência que o grep acharia; leitura limitada a 10 mil registros por busca, com aviso (`capped`). Fontes são um trait (`GrepSource`) — fonte nova não muda a ferramenta.
- [x] 🟢 Concluído — Chat da IA com **`@` e desfazer**: digitar `@` no campo abre a lista de dispositivos, monitores, containers Docker e fontes de dados (logs, alertas, checagens, Docker) — `GET /api/ai/mentions`, busca sem caixa em memória, igual nos dois bancos; o que foi marcado vai para o prompt como alvo da pergunta, com prioridade sobre o contexto da tela, e fica anotado no histórico. Ao lado do copiar, **desfazer** volta a conversa para antes da pergunta (interrompendo a resposta em andamento) e devolve o texto e as marcações ao campo. Quando não está claro de qual equipamento vem a informação, a IA chama `ask_user`: a pergunta aparece com opções clicáveis e a rodada termina ali, sem outra chamada ao provedor (ferramenta `Interactive`, fora das rotinas automáticas). Novos: `get_docker_containers` (estado e rede, nunca variáveis de ambiente) e a fonte `docker` do `grep` (logs de um container).
- [x] 🟢 Concluído — Economia de tokens no chat da IA: um "Oi" custava ~22 mil tokens (o método mandava começar consultando e o catálogo inteiro de ferramentas ia em cada uma das quatro rodadas). Agora cortesia ("oi", "bom dia, tudo bem?", "valeu") vai sem ferramenta e com prompt de uma linha; pergunta de verdade leva só o núcleo (inventário, alertas, grep, documentação, `ask_user`) e os grupos que ela obviamente pede ("gráfico" já traz os gráficos), com os demais listados no prompt para a IA carregar com `load_tools` — invisível na tela; o método pede o mínimo de consultas, todas na mesma rodada; resultado de ferramenta acima de 8 mil caracteres chega cortado com aviso. Rotinas automáticas continuam com o catálogo inteiro. Núcleo: ~5,7 mil caracteres por rodada, contra ~14 mil do catálogo inteiro.
- [x] 🟢 Concluído — IA com acesso ao resto do sistema e prefixo estável para o cache do provedor. **Docker multi-host**: `get_docker_containers` e o `grep` (fonte `docker`) aceitam `host` — a central ou qualquer agente remoto conectado (ADR 011), antes só a Engine local era visível; novos `get_docker_hosts` e `get_docker_usage` (CPU/memória agora pelo cache de métricas, ou histórico da telemetria com resumo avg/max/último do servidor e ranking de containers — nunca os pontos). **Grupo `platform`**: `get_platform_status` com `area` (agentes, VPN com peers e handshake, descoberta, redes/sites/DNS, janelas de manutenção, auditoria, notificações) — um schema no lugar de sete — e `get_topology` (uplink e vizinhos com as portas dos dois lados, ou os enlaces fora de `up`). **Cache de prefixo**, no modelo do Claude Code: o system prompt ficou igual a conversa inteira (data sem hora, catálogo completo mesmo depois de carregar), e a hora, os `@` e o contexto da tela viajam num bloco `<contexto>` junto da última pergunta; grupo carregado entra no **fim** da lista de ferramentas e continua carregado nas perguntas seguintes (`toolGroups` no evento `usage`, devolvido no `contextHint`), zerando na compactação; `cache_control` no system prompt e na última pergunta para Anthropic/Gemini via OpenRouter; `cachedTokens` lido de `prompt_tokens_details`/`prompt_cache_hit_tokens` e enviado à tela. Resultado grande deixou de ser cortado no meio do JSON: as listas mais pesadas encolhem pela metade até caber, com `_omitidos` contando o que saiu. Rotinas automáticas deixaram de receber `load_tools` e o catálogo, inúteis com tudo carregado.
- [x] 🟢 Concluído — IA com **ações em containers Docker** (`docker_container_action`: iniciar, parar, reiniciar — remover e atualizar ficam de fora) na central e nos agentes remotos cuja política local (`AGENT_ALLOW`) inclui `lifecycle`. Configuração própria `containerActions` em três modos: `off`, `confirm` (**padrão** — a IA propõe e o usuário confirma no chat) e `auto` (executa na hora). A ação automática fica na auditoria em nome de quem conversa (o chat passou a identificar o usuário), e as rotinas proativas nunca mexem em container. Novo tipo `ToolKind::ContainerAction`; o prompt ganha a regra do modo ativo.
- [x] 🟢 Concluído — Compactação que só roda quando compensa: um "oi" num Ollama com `num_ctx` padrão (4.096) disparava resumo de "oi/Olá" (3,3 mil → 3,4 mil tokens, mais uma chamada ao modelo), porque só instruções + ferramentas já passavam da meta. Agora o trecho resumido precisa pesar ao menos dois resumos (`worth_folding`; o pedido explícito da tela continua valendo); quando a parte fixa é que não cabe, os grupos de ferramentas herdados de perguntas anteriores saem antes de qualquer resumo, e se nem assim couber a resposta traz um aviso (`notice`) com a janela real e como aumentá-la (`OLLAMA_CONTEXT_LENGTH`).
...
- [x] **Assistente IA, Docker e endereços — rodada de usabilidade** 🟢 Concluído
  - **Chat da IA:** cada resposta mostra o modelo que de fato respondeu (o roteador `openrouter/free` escolhe um por pergunta), tokens enviados/recebidos e a velocidade de geração em tokens/s; o campo de digitação mostra o uso da janela de contexto (estimada pelo modelo, `services/ai/context_window.rs`), âmbar a partir de 75%. Barras do campo e blocos de código com contraste corrigido: `bg-surface-variant` é cinza-claro no tema escuro.
  - **Resumo da rede ("Gerar agora"):** a requisição roda o laço do agente inteiro e estourava o timeout padrão de 15 s do `apiService`, aparecendo como "Falha de conexão". Timeout próprio de 12 min e aviso de que pode demorar.
  - **IA e regras de alerta:** grupo de ferramentas `alert_rules` — `get_alert_rules_guide` (guia em `services/ai/knowledge/regras_de_alerta.md`, também na busca da documentação), `list_alert_rules` (com disparos em 24 h e regra ruidosa), `explain_alert` (regra, alvo, fatos que casaram), `create_alert_rule` e `delete_alert_rule`, ambas com confirmação e aviso de que excluir apaga o histórico da regra. Criar/alterar/excluir regra saiu do controller para `services/alerts/rules.rs`, caminho único da tela e da IA.
  - **Docker:** "Consumo geral" com CPU e RAM empilhadas por container, linha do total e tooltip por horário (`StackedUsageChart`); indicador "Ao vivo" ligado ao estado real do SSE, junto dos dados em vez da barra de abas; coluna "Recursos" em grade e só com a memória usada; contagem de containers por rede corrigida (o `GET /networks` da Engine não preenche `Containers` — a conta sai da lista de containers).
  - **Janela de contexto real e compactação:** a janela vem do provedor — OpenRouter (`/models`, menor entre `context_length` e `top_provider`), Ollama (`/api/ps` do modelo carregado, senão `num_ctx`/padrão 4096 como presumido) e OpenCode (catálogo `models.dev`) — com cache (6 h; 60 s no Ollama) e a estimativa pelo nome como reserva, sinalizada na tela (`services/ai/context_window/`). A conversa é compactada quando a entrada chega a `min(80% W, 90% W − saída reservada)`: as mensagens antigas viram resumo feito pelo próprio modelo e o uso volta para ~50%; o teto de 90% dá 10% de margem ao erro da estimativa (3 caracteres/token, conservadora). Entre rodadas de ferramentas, resultados antigos são podados para não passar do teto. Sem resumo do provedor, as mensagens saem com aviso. A tela guarda o resumo na conversa, mostra o divisor "Contexto compactado" e permite pedir a compactação na próxima pergunta (`harness/compaction.rs`).
  - **Endereços deste servidor:** tipo "Domínio" (nome DNS, com opção de proxy HTTPS); sem padrão marcado, é o padrão dos comandos de instalação dos servidores remotos, e o seletor mostra qual é o padrão.
