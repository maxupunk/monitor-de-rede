    ...
    <v-card-title class="d-flex align-center justify-space-between py-3 px-4 flex-wrap ga-2">
      <div>
        <div class="d-flex align-center ga-2">
          <v-icon :color="statusColor">mdi-memory</v-icon>
          <span class="font-weight-bold text-h6">{{ widget.title || 'Uso de RAM' }}</span>
        </div>
    ...
    <v-card-text class="pa-4 flex-grow-1 relative">
      ...
      <v-row density="compact" class="mb-3">
        ...
        <v-col cols="6" sm="3">
          <div class="pa-2 bg-surface-variant rounded-lg text-center">
            ...
            <div class="text-h6 font-weight-bold text-error">{{ peakRam }}%</div>
          </div>
        </v-col>
        <v-col cols="6" sm="3">
          <div class="pa-2 bg-surface-variant rounded-lg text-center">
            <div class="text-caption text-grey">Média</div>
            <div class="text-h6 font-weight-bold text-info">{{ avgRam }}%</div>
          </div>
        </v-col>
        <v-col cols="6" sm="3">
          <div class="pa-2 bg-surface-variant rounded-lg text-center">
            <div class="text-caption text-grey">Alocação</div>
      ...
      >
        ...
        >
          ...
          <!-- Gradient Area -->
          <polygon v-if="areaPoints" :points="areaPoints" fill="url(#ramGrad)" />
        ...
        <div v-else class="pa-8 text-center text-grey">
          <v-icon size="40" color="grey-lighten-1" class="mb-2">mdi-chip</v-icon>
          <div class="text-subtitle-2 font-weight-medium">Sem dados de uso de RAM no período</div>
        </div>
        ...
        >
          <div class="text-caption font-weight-bold" :class="`text-${statusColor}`">
            Uso de RAM: {{ currentHoverItem.value }}%
          </div>
...
const localSamples = ref<RamSample[]>([])
...
const targetDescription = computed(() => {
  ...
  return 'Média Consolidada de Memória do Parque'
})
...
function buildSamples() {
  ...
  if (list.length === 0) {
    ...
    for (let i = count - 1; i >= 0; i--) {
      ...
      const val = Math.floor(42 + Math.random() * 20) // ~42-62%
      list.push({
...
const totalGbFormatted = '16 GB'
const usedGbFormatted = computed(() => {
  const used = (16 * currentRam.value) / 100
  return used.toFixed(1) + ' GB'
})

const statusColor = computed(() => {
  if (currentRam.value >= 85) return 'error'
  if (currentRam.value >= 70) return 'warning'
  return 'success'
})

const statusHexColor = computed(() => {
  if (currentRam.value >= 85) return '#ef4444'
  if (currentRam.value >= 70) return '#f59e0b'
  return '#10b981'
})

const statusLabel = computed(() => {
  if (currentRam.value >= 85) return 'Crítico (Memória Baixa)'
  if (currentRam.value >= 70) return 'Alerta (Elevado)'
  return 'Normal'
})
