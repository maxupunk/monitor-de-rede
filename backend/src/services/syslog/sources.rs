...
impl SourceRegistry {
    ...
    pub fn record(
        ...
        if fontes.len() >= MAX_SOURCES && !fontes.contains_key(source_ip) {
...
fn despeja_menos_vista(fontes: &mut HashMap<String, SeenSource>) {
    if let Some(chave) = fontes
        ...
        .map(|fonte| fonte.source_ip.clone())
    {
        fontes.remove(&chave);
    }
}
...
mod tests {
    ...
    fn a_fonte_desconhecida_conta_o_que_foi_descartado() {
        ...
        for _ in 0..3 {
            registro.record("203.0.113.9", &Resolution::Unknown, None, "oi", agora());
        }
    ...
    fn a_reclassificacao_e_registrada_sem_perder_a_contagem() {
        ...
        let registro = SourceRegistry::create();
        registro.record("192.168.88.1", &Resolution::Unknown, None, "a", agora());
        registro.record("192.168.88.1", &Resolution::Device(4), None, "b", agora());
        let fontes = registro.list();
    ...
    fn o_registro_tem_teto_e_despeja_o_ruido_primeiro() {
        ...
        for _ in 0..50 {
            registro.record("10.0.0.1", &Resolution::Unknown, None, "sinal", agora());
        }
        for indice in 0..(MAX_SOURCES + 100) {
            let ip = format!("10.9.{}.{}", indice / 256, indice % 256);
            registro.record(&ip, &Resolution::Unknown, None, "ruído", agora());
        }
        let fontes = registro.list();
    ...
    fn a_mensagem_guardada_e_truncada() {
        ...
        registro.record("10.0.0.1", &Resolution::Unknown, None, &longa, agora());
